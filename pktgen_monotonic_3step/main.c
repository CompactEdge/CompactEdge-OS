...
int pktgen_client_run(struct pktgen_socks *socks, char* interface, int64_t sample, FILE *fp)
{
    ...
    while(sample)
    {
        ...
        // Write to file
        fprintf(fp, "xdp_rx: %"PRId64"\n", msg.xdp_recv);
        fprintf(fp, "tc_ingress:  %"PRId64"\n", msg.tc_ingress);
        fprintf(fp, "curr_time: %"PRId64"\n\n", curr_timestamp.tv_sec * 1000000000UL + curr_timestamp.tv_nsec);
	fprintf(fp, "diff_xdp_to_tc: %"PRId64"\n", msg.tc_ingress - msg.xdp_recv);
	fprintf(fp, "diff_tc_to_app: %"PRId64"\n", curr_timestamp.tv_sec * 1000000000UL + curr_timestamp.tv_nsec - msg.tc_ingress);
	fprintf(fp, "diff: %"PRId64"\n\n", curr_timestamp.tv_sec * 1000000000UL + curr_timestamp.tv_nsec - msg.xdp_recv);
    }
    return 0;
}
