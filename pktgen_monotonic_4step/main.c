...
int pktgen_client_run(struct pktgen_socks *socks, char* interface, int64_t sample, FILE *fp, int run_time, int warmup, int snapshot_threshold, int *arr_cpu, int16_t cpu_cnt, bool cont_snapshot)
{
    ...
    // Allocate buffers
    write_buf_global = malloc(41943040);
    memset(write_buf_global, 0, 41943040);
    write_buf_local = malloc(20971520);
    memset(write_buf_local, 0, 20971520);
    // Initialize mutex
...
//        fprintf(set_ftrace_pid, "%ld", (long)getpid());
    ...
    while(sample)
    {
        ...
        write_buf_pos += sprintf(write_buf_local + write_buf_pos, "iptables_to_cont: %"PRId64"\n", msg.xdp_cont_recv - msg.iptables_out);
        write_buf_pos += sprintf(write_buf_local + write_buf_pos, "cont_to_app: %"PRId64"\n\n", curr_time - msg.xdp_cont_recv);
